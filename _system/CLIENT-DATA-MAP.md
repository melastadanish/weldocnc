# CLIENT-DATA-MAP — WeldoCNC
> Single source of truth for all verified specs, claims, and numbers used in content.
> Every number in every page must trace back to this file. If it is not here, do not write it.

---

## Company Identity

| Field | Verified Value | Source |
|---|---|---|
| Brand name (new site) | WeldoCNC | Client confirmed |
| Legal/operating name | Weldo Machining | weldomachining.com |
| Founded | 2012 | weldomachining.com/about |
| Focus (new site) | CNC machining only — no welding, sheet metal, 3D printing, injection molding | Client confirmed |
| Location | Dongguan City, Guangdong, China | weldomachining.com |
| Certifications | ISO 9001:2015 | weldomachining.com |

---

## Scale & Capacity

| Claim | Verified Number | Source |
|---|---|---|
| Employees | 120+ professional technicians | weldomachining.com/about |
| CNC machines | 100+ | weldomachining.com/cnc-machining |
| Inspection/testing tools | 20+ | weldomachining.com/cnc-machining |
| Quality inspectors | 30+ | weldomachining.com/cnc-machining |
| Annual sales | Exceeding $10 million USD | weldomachining.com/about |
| Countries/regions served | 100+ | weldomachining.com/about |
| Operator experience | Average 10+ years | weldomachining.com/cnc-machining |

> **DO NOT USE** these metrics until client confirms they still apply to the CNC-only entity:
> "120,000+ parts manufactured" · "25,000+ projects delivered" · "6,000+ complex projects"
> These may include non-CNC services and could overstate CNC-only output.

---

## Tolerances — By Process

| Process | Standard Tolerance | High-Precision Tolerance | Notes |
|---|---|---|---|
| CNC Milling | ±0.05 mm | ±0.005 mm | Depends on part geometry and material |
| CNC Turning | ±0.05 mm | ±0.005 mm | Depends on part geometry and material |
| 5-Axis CNC | ±0.05 mm | ±0.003 mm | Complex geometry capability |
| General (metals/plastics) | ±0.08 mm – ±0.2 mm | ±0.001 mm – ±0.05 mm | Per weldomachining.com/cnc-machining |
| General standard | ISO 2768-m (metals) · ISO 2768-c (plastics) | Industry standard reference |
| Achievable precision | Down to ±0.0127 mm (±0.0005 inches) under optimal conditions | Industry benchmark |

> **USE IN CONTENT:** "Tolerances to ±0.005 mm" is the confirmed headline spec.
> "Precision machining to ±0.003 mm" is confirmed for 5-axis only.
> ISO 2768-m/c is the industry standard — safe to reference as the baseline.
> Do not claim sub-micron tolerances without specific client confirmation.

---

## Lead Times

| Order Type | Lead Time | Source |
|---|---|---|
| Prototype (simple parts) | 1 day (fast track) | Industry benchmark |
| Prototype (standard) | 1–5 business days | weldomachining.com/cnc-machining |
| Small-batch production | 5–15 business days | weldomachining.com/cnc-machining |
| Complex parts | 7–8 business days | Industry benchmark |
| Full production runs | 2–5 weeks | weldomachining.com/cnc-machining |
| Delivery (North America / Europe) | 3–7 days shipping after production | weldomachining.com |
| Quote turnaround | 24 hours | design.md (hero trust badge) |
| On-time delivery rate | 98% | weldomachining.com |

---

## Quality & Inspection

| Claim | Verified Detail | Source |
|---|---|---|
| Certification | ISO 9001:2015 | weldomachining.com |
| Annual pass/yield rate | 97.48% | weldomachining.com/about |
| Inspection equipment | CMMs, XRF analyzers, 20+ tools | weldomachining.com |
| Quality process | Pre-production · In-production · Final inspection | weldomachining.com |
| Inspection report | Full dimensional inspection report with every shipment | [CLIENT TO CONFIRM — standard practice] |

---

## CNC Processes Offered (New Site Scope — CNC Only)

| Process | Confirmed | Notes |
|---|---|---|
| 3-Axis CNC Milling | Yes | |
| 4-Axis CNC Milling | Yes | |
| 5-Axis CNC Milling | Yes | |
| CNC Turning | Yes | |
| Turn-Mill (compound) | Yes | |
| Surface Grinding | Yes | Include — CNC-adjacent precision process |
| Centerless Grinding | Yes | Include — CNC-adjacent precision process |
| Internal Grinding | Yes | Include — CNC-adjacent precision process |
| Wire EDM | Yes | Include — CNC-adjacent precision process |
| Sheet metal fabrication | **Exclude** | Not CNC — do not feature on new site |
| 3D printing | **Exclude** | Not CNC — do not feature on new site |
| Injection molding | **Exclude** | Not CNC — do not feature on new site |
| Aluminum extrusion | **Exclude** | Not CNC — do not feature on new site |
| Vacuum casting | **Exclude** | Not CNC — do not feature on new site |
| Die casting | **Exclude** | Not CNC — do not feature on new site |

---

## Materials — Confirmed

### Metals
| Material | Grades Confirmed | Notes |
|---|---|---|
| Aluminum | 6061, 6061-T6, 7075, 7075-T6, 2024, 5052, 5083, 6063, 6082, MIC-6 | ADC12/A380 for reference only — die cast grade |
| Stainless Steel | 201, 303, 304, 316, 316L, 430, 431, 440C, 17-4PH (SUS630) | |
| Carbon Steel | 1018, 1020, 1025, 1045, A36, cold rolled steel | |
| Alloy Steel | 1215, 4130, 4140, 4340, 5140, bearing steel, spring steel | |
| Tool Steel | H13, D2, high speed steel | |
| Brass | C360 [CLIENT TO CONFIRM — industry standard grade] | |
| Bronze | Tin bronze [CLIENT TO CONFIRM grade] | |
| Copper | C101, C103, C110, beryllium copper | |
| Titanium | TA1 (Grade 1), TA2 (Grade 2), TC4 / Ti-6Al-4V (Grade 5) | |
| Magnesium | AZ31B, AZ91D | |

### Plastics
| Material | Notes |
|---|---|
| ABS | |
| POM (Delrin) | |
| PEEK | |
| PMMA (Acrylic) | |
| PC (Polycarbonate) | |
| Nylon / PA | |
| PTFE (Teflon) | |
| HDPE | |
| PP (Polypropylene) | |
| FR4 | |
| PVC | |
| HIPS | |
| LDPE | |
| PBT | |
| PPA | |
| PAI | |
| PET | |
| PPS | |
| PS | |
| UPE | |
| Bakelite | |

> Total: "100+ materials" is confirmed per weldomachining.com/about.

---

## Surface Finishes — Confirmed Categories

> weldomachining.com states "50+ surface finishes". The following are confirmed as offered:

| Finish | Compatible Materials | Notes |
|---|---|---|
| As-machined | All | Default finish — Ra 3.2 µm [CLIENT TO CONFIRM Ra value] |
| Anodizing | Aluminium | Type II / Type III [CLIENT TO CONFIRM colour options] |
| Powder Coating | Metals | [CLIENT TO CONFIRM colour range] |
| Passivation | Stainless steel | Removes free iron, improves corrosion resistance |
| Nickel Plating | Metals | |
| Bead Blasting (Sand Blasting) | Metals, some plastics | Matte uniform texture |
| Black Oxide | Steel, stainless steel | |
| Electropolishing | Stainless steel, aluminium | |
| Brushed Finish | Metals | Directional grain texture |
| Polishing | Metals | Mirror or semi-mirror finish |
| Electroplating | Metals | [CLIENT TO CONFIRM plating types] |
| Alodine (Chromate conversion) | Aluminium | Corrosion resistance, paint adhesion |
| Heat Treatment | Steel, tool steel | Hardening, stress relief |
| Tumbling | Metals | Deburring, edge breaking |

---

## Part Size Limits — By Process

| Process | Maximum Envelope | Minimum Envelope | Notes |
|---|---|---|---|
| CNC Milling | 4000 × 1500 × 600 mm | 5 × 5 × 5 mm | Industry benchmark — [CLIENT TO CONFIRM machine envelope] |
| CNC Turning | Ø200 × 500 mm length | Ø2 × 2 mm | Industry benchmark — [CLIENT TO CONFIRM] |
| Minimum wall thickness | 0.5 mm | — | Below this: deflection risk |
| Minimum end mill diameter | 0.5 mm | — | Feature size limit |
| Minimum drill size | 1 mm | — | |
| Maximum drill depth | 12× diameter | — | Standard drill depth rule |
| Thread range | Ø1.5 mm and up | — | Below Ø5mm: 3× depth · Ø5mm+: 4–6× depth |

## Order Volumes

| Tier | Quantity | Notes |
|---|---|---|
| Prototyping | 1–100 pieces | |
| Low volume production | 101–10,000 pieces | |
| High volume production | 10,001+ pieces | |
| Minimum order quantity | No MOQ | Confirmed — single part orders accepted |

---

## Industries Served

| Industry | Key Part Types | Notes |
|---|---|---|
| Aerospace | Fuselage frames, turbine disks, compressor blades, avionics housings, fasteners | |
| Medical | Bone screws, joint implants, surgical instruments, medical equipment housings | |
| Automotive | Engine blocks, crankshafts, transmission parts, EV motor housings, battery trays | |
| Electronics / Consumer | Phone frames, laptop casings, heat dissipation components, brackets | |
| Industrial Machinery | Gears, shafts, hydraulic components, structural load-bearing parts | |
| Robotics & Automation | Robotic arm components, joint housings, linear motion modules, gripper bodies | |
| Energy & Power | Turbine casings, wind power components, pump and valve bodies | |

---

## File Formats Accepted

| Format | Type |
|---|---|
| .stp / .step | 3D — preferred |
| .igs / .iges | 3D |
| .prt | 3D (Pro/E, NX) |
| .sldprt | 3D (SolidWorks) |
| .sat | 3D (ACIS) |
| .x_t | 3D (Parasolid) |
| .stl | 3D (mesh) |
| .pdf | 2D drawing |
| .dwg / .dxf | 2D drawing [CLIENT TO CONFIRM DXF support] |
| .jpg / .jpeg / .png | Reference images |
| .zip / .rar | Compressed packages |

---

## Approved Authority Statements

Use these verbatim. Do not paraphrase in ways that change the meaning.

| Claim | Approved Copy | Source |
|---|---|---|
| Certification | "ISO 9001:2015 certified" | Confirmed |
| Pass rate | "97.48% annual pass rate" | weldomachining.com/about |
| On-time delivery | "98% on-time delivery rate" | weldomachining.com |
| Scale | "100+ CNC machines" | weldomachining.com/cnc-machining |
| Team | "120+ professional technicians" | weldomachining.com/about |
| Quote speed | "Quote within 24 hours" | design.md |
| Countries | "Customers in 100+ countries" | weldomachining.com/about |
| Experience | "Operating since 2012" | weldomachining.com/about |
| Materials | "100+ machinable materials" | weldomachining.com/about |
| Finishes | "50+ surface finish options" | weldomachining.com/cnc-machining |

---

## Claims — DO NOT USE

| Banned Claim | Reason |
|---|---|
| "20+ years experience" | Founded 2012 — that's 13 years as of 2025. Not 20+. |
| Any non-CNC service (3D printing, injection molding, etc.) | Outside new site scope |
| "Award-winning" | Not confirmed |
| "Trusted by thousands" | Unverified count |
| Testimonials or star ratings | Only use if client supplies real data |
| Tolerance tighter than ±0.001 mm | Not confirmed |

---

## Items Requiring Client Confirmation

- [ ] Exact Ra surface roughness values by finish type (as-machined, bead blasted, polished)
- [ ] Anodizing colour options and whether Type II and Type III are both offered
- [ ] Electroplating types offered (zinc, chrome, gold, etc.)
- [ ] DXF file format — confirm support
- [ ] Whether dimensional inspection report is included standard with every order
- [ ] Whether the 100+ countries figure applies to CNC business specifically
- [ ] Actual machine envelope for milling and turning (confirm vs industry benchmark)
- [ ] Brass grade — confirm C360 or specify grade used
- [ ] Bronze grade — confirm specific alloy
- [ ] Payment terms and accepted currencies
- [ ] Whether ISO 13485 (medical) or IATF 16949 (automotive) certifications are held [CLIENT TO CONFIRM — high value if true]
